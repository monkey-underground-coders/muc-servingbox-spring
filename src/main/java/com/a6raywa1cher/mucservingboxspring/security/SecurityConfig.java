...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
			...
			.regexMatchers("^/user/reg\\?role=(CUSTOMER|MODERATOR|ADMIN)$").hasRole("ADMIN")
			.regexMatchers("^/user/reg$", "^/user/fast_reg$").permitAll()
			.antMatchers(HttpMethod.GET, "/user/").hasRole("MODERATOR")
			.antMatchers(HttpMethod.GET, "/user/{uid:[0-9]+}").hasRole("MODERATOR")
			.antMatchers("/user/change_password").hasRole("CUSTOMER")
			.antMatchers(HttpMethod.POST, "/user/{uid:[0-9]+}/set_password", "/user/{uid:[0-9]+}").access("@mvcAccessChecker.checkUserWrite(#uid, authentication)")
			...
			.antMatchers(HttpMethod.GET, "/shop_item/**").permitAll()
			.antMatchers("/shop_item/**").hasRole("MODERATOR")
			.antMatchers(HttpMethod.DELETE, "/order/{id:[0-9]+}").access("@mvcAccessChecker.checkOrderWrite(#id, authentication)")
			.antMatchers(HttpMethod.PUT, "/order/**").hasRole("MODERATOR")
			.antMatchers(HttpMethod.POST, "/order/**").hasRole("CUSTOMER")
			.antMatchers(HttpMethod.GET, "/order/{id:[0-9]+}").access("@mvcAccessChecker.checkOrder(#id, authentication)")
			.antMatchers(HttpMethod.GET, "/order/").hasRole("CUSTOMER")
			.antMatchers(HttpMethod.GET, "/order/**").hasRole("MODERATOR")
